...
SELECT TOP 3 Customer_Name, COUNT(Order_Quantity) AS Quantity
FROM Combined_Table
...
SELECT TOP 3 CD.Customer_Name, COUNT(MF.Order_Quantity)
FROM market_fact MF, cust_dimen CD
...
WHERE MONTH(Order_Date) = 1 and YEAR(Order_Date) = 2011  AND A.Cust_id = B.Cust_id ) 
...
FROM Combined_Table A
WHERE EXISTS
				(SELECT CUST_ID, COUNT(Ord_id) FROM COMBINED_TABLE B WHERE A.Cust_id = B.Cust_id GROUP BY CUST_ID HAVING COUNT(Ord_id)>=3)
ORDER BY Cust_id, Customer_Name, Order_Date



------------------------------
SELECT DISTINCT CUST_ID, Order_Date, Customer_Name, ORD_ID, FIRST_PURCH, DATEDIFF (day, FIRST_PURCH, order_date) DAYS_ELAPSED
FROM
(SELECT CUST_ID, Order_Date, Customer_Name, ORD_ID,
		MIN(ORDER_DATE) OVER (PARTITION BY CUST_ID) FIRST_PURCH, 
		DENSE_RANK () OVER (PARTITION BY CUST_ID ORDER BY ORDER_DATE) NUMBER
FROM Combined_Table) A
WHERE NUMBER = 3
